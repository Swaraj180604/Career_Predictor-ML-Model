...
:root{--bg:#07070e;--surface:#0f0f1a;--card:#14141f;--card2:#1b1b2a;--border:#252538;
      --accent:#7c3aed;--a2:#06b6d4;--a3:#f59e0b;--a4:#10b981;--a5:#ef4444;
      --text:#e2e8f0;--muted:#5a6478;--glow:rgba(124,58,237,.4);--glow2:rgba(6,182,212,.3);}
...
.orb{position:absolute;border-radius:50%;filter:blur(80px);opacity:.1;animation:drift 18s ease-in-out infinite alternate;}
...
/* ── HERO ── */
.hero{text-align:center;padding:2.8rem 1rem 1.5rem;position:relative;}
...
@keyframes pulse-border{0%,100%{border-color:rgba(6,182,212,.3);box-shadow:none;}
...
.live-dot{width:6px;height:6px;border-radius:50%;background:var(--a4);animation:blink 1.2s ease-in-out infinite;}
...
.conf-meter-wrap{margin:.8rem 0;}
.conf-meter-bg{background:var(--surface);border-radius:999px;height:8px;overflow:hidden;}
.conf-meter-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--a2));
  box-shadow:0 0 10px var(--glow2);transition:width 1s cubic-bezier(.4,0,.2,1);}
...
.alt-chip{background:var(--surface);border:1px solid var(--border);border-radius:999px;
  padding:.28rem .9rem;font-size:.75rem;font-family:'Space Mono',monospace;color:var(--muted);transition:all .15s;}
.alt-chip:hover{border-color:var(--a2);color:var(--a2);}

/* ── SECTION TITLE ── */
.section-title{font-size:1.1rem;font-weight:700;margin-bottom:1rem;display:flex;align-items:center;gap:.5rem;}
.dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--accent);box-shadow:0 0 8px var(--accent);}
...
.quiz-progress{font-family:'Space Mono',monospace;font-size:.7rem;color:var(--muted);text-align:right;margin-bottom:.5rem;}
.progress-bar-wrap{background:var(--surface);border-radius:999px;height:4px;margin-bottom:1.2rem;}
.progress-bar{height:100%;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--a2));transition:width .4s ease;}

/* ── EXPLORER CARDS ── */
.career-exp-card{background:var(--card);border:1px solid var(--border);border-radius:16px;
  padding:1.2rem 1rem;text-align:center;cursor:pointer;transition:all .2s;}
.career-exp-icon{font-size:2.2rem;margin-bottom:.5rem;}
.career-exp-name{font-size:.88rem;font-weight:700;}

/* ── BUTTONS ── */
div.stButton>button{width:100%;background:linear-gradient(135deg,var(--accent),var(--a2))!important;
  ...
  border-radius:14px!important;padding:.8rem 2rem!important;cursor:pointer!important;
...
/* ── TABS ── */
.stTabs [data-baseweb="tab-list"]{background:var(--surface)!important;border-radius:12px;padding:.3rem;gap:.2rem;}
.stTabs [data-baseweb="tab"]{background:transparent!important;color:var(--muted)!important;
  border-radius:8px!important;font-family:'Syne',sans-serif!important;font-size:.85rem!important;font-weight:600!important;}
...
# ── Animated BG ────────────────────────────────────────────────────────────────
st.markdown('<div class="bg-anim"><div class="orb orb1"></div><div class="orb orb2"></div><div class="orb orb3"></div></div>', unsafe_allow_html=True)

# ── Load models ────────────────────────────────────────────────────────────────
@st.cache_resource
def load_models():
    pipe = joblib.load("models/career_model.joblib")
...
CAREER_SALARY = {
    "Software Engineer":"₹8L–₹35L","Data Scientist":"₹10L–₹40L","Web Developer":"₹5L–₹22L",
    "Cybersecurity Analyst":"₹8L–₹30L","AI/ML Engineer":"₹12L–₹50L","Database Administrator":"₹6L–₹25L",
    "Network Engineer":"₹5L–₹20L","UI/UX Designer":"₹5L–₹25L","Product Manager":"₹12L–₹45L","DevOps Engineer":"₹8L–₹32L",
}
CAREER_DEMAND = {
    "Software Engineer":"🔥 Very High","Data Scientist":"🔥 Very High","Web Developer":"✅ High",
    "Cybersecurity Analyst":"🔥 Very High","AI/ML Engineer":"🚀 Explosive","Database Administrator":"✅ Moderate",
    "Network Engineer":"✅ Moderate","UI/UX Designer":"✅ High","Product Manager":"✅ High","DevOps Engineer":"🔥 Very High",
}
SKILL_LABELS = {
    "math":"🔢 Mathematics","logic":"🧠 Logic","creativity":"🎨 Creativity",
    "communication":"💬 Communication","leadership":"👑 Leadership","problem_solving":"🔍 Problem Solving",
    "programming":"💻 Programming","data_analysis":"📊 Data Analysis","design":"✏️ UI/UX Design",
...
QUIZ_QUESTIONS = [
    ...
    {"q":"🌐 Which tech area sounds most appealing?",
     "opts":["Building scalable backend systems","AI and predictive models","Network security and protocols","Agile product roadmaps"],
     "skill_map":[{"programming":2,"logic":1},{"math":2,"data_analysis":1},{"networking":2,"logic":1},{"management":2,"leadership":1}]},
]

# ── Session state init ─────────────────────────────────────────────────────────
for k, v in {"quiz_step":0,"quiz_scores":{k:5 for k in FEATURES},"quiz_done":False,
              "explorer_career":None,"compare_a":"Software Engineer","compare_b":"Data Scientist"}.items():
...
def make_radar(vals, label, color, fill):
...
def radar_layout(h=320):
    return dict(
        polar=dict(bgcolor='rgba(0,0,0,0)',
                   radialaxis=dict(visible=True,range=[0,10],tickfont=dict(color='#64748b',size=8),gridcolor='#252538'),
                   angularaxis=dict(tickfont=dict(color='#94a3b8',size=9),gridcolor='#252538')),
        paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',
        showlegend=True,legend=dict(font=dict(color='#94a3b8',size=11),bgcolor='rgba(0,0,0,0)'),
...
st.markdown("""
<div class="hero">
  <div class="hero-badge">✦ AI-Powered · 88% Accuracy · Live Prediction · 4 Modes</div>
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    col_sliders, col_viz = st.columns([1, 1.1], gap="large")

    with col_sliders:
        st.markdown('<div class="section-title"><span class="dot"></span> Rate Your Skills <span style="color:var(--muted);font-size:.8rem;font-weight:400"> — drag any slider for instant AI result</span></div>', unsafe_allow_html=True)
        ...
        for col, (pname, pvals) in zip([pc1,pc2,pc3,pc4], presets.items()):
            with col:
                if st.button(pname, key=f"preset_{pname}"):
                    for k, v in pvals.items():
                        st.session_state[f"live_{k}"] = v
                    st.rerun()
        ...
        for i, key in enumerate(FEATURES):
            ...
            with col:
                values[key] = st.slider(
                    SKILL_LABELS[key], 1, 10,
                    st.session_state.get(f"live_{key}", 5),
                    key=f"live_{key}"
                )
        ...
        if st.button("↺  Reset All to 5", key="reset_all"):
            for k in FEATURES:
                st.session_state[f"live_{k}"] = 5
            st.rerun()

    # ── RIGHT COLUMN: live results ────────────────────────────────────────
    with col_viz:
        results      = run_prediction(values)
        ...
        # ── Result card ─────────────────────────────────────────────────
        alt_html = "".join([f'<span class="alt-chip">{CAREER_ICONS[c]} {c} · {p:.0f}%</span>' for c,p in top5[1:]])
        st.markdown(f"""
        <div class="live-badge"><span class="live-dot"></span> LIVE PREDICTION — updates instantly</div>
        <div class="result-card">
          ...
          <div class="conf-meter-wrap">
            <div class="conf-meter-bg"><div class="conf-meter-fill" style="width:{top_conf:.1f}%"></div></div>
          </div>
        ...
        # ── Radar: You vs Ideal ──────────────────────────────────────
        fig_r = go.Figure()
        fig_r.add_trace(make_radar(values, "You", "#7c3aed", "rgba(124,58,237,0.15)"))
        fig_r.add_trace(make_radar(ideal, f"Ideal {top_career}", "#06b6d4", "rgba(6,182,212,0.08)"))
        fig_r.update_layout(**radar_layout(300))
        st.plotly_chart(fig_r, use_container_width=True, config={'displayModeBar':False})

        # ── All-10 probability bar chart ─────────────────────────────
        all_c  = [c for c,_ in results]
        all_p  = [p for _,p in results]
        colors = ['#7c3aed' if i==0 else '#252538' for i in range(10)]
        fig_b  = go.Figure(go.Bar(
            x=all_p, y=[f"{CAREER_ICONS[c]} {c}" for c in all_c],
            orientation='h', marker=dict(color=colors, line=dict(width=0)),
            text=[f"{p:.1f}%" for p in all_p], textposition='inside',
        ...
        fig_b.update_layout(
            ...
            margin=dict(l=10,r=20,t=28,b=10), height=320,
            title=dict(text="All Career Match Probabilities",font=dict(color='#64748b',size=12),x=0)
        )
        st.plotly_chart(fig_b, use_container_width=True, config={'displayModeBar':False})

        # ── Salary + demand expander ──────────────────────────────────
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown('<div class="section-title"><span class="dot"></span> Skill Discovery Quiz — 6 questions that auto-fill your profile</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.quiz_done:
        ...
        qr  = run_prediction(qs)
        qtop, qconf = qr[0]

        alt_q = "".join([f'<span class="alt-chip">{CAREER_ICONS[c]} {c} · {p:.0f}%</span>' for c,p in qr[1:5]])
        st.markdown(f"""
        <div class="result-card">
          ...
          <div class="conf-meter-wrap">
            <div class="conf-meter-bg"><div class="conf-meter-fill" style="width:{qconf:.1f}%"></div></div>
          </div>
        ...
        with qc1:
            st.markdown('<div class="section-title" style="margin-top:1.2rem"><span class="dot"></span> Quiz Skill Profile vs Ideal</div>', unsafe_allow_html=True)
            fig_qr = go.Figure()
            fig_qr.add_trace(make_radar(qs, "Your Profile", "#7c3aed", "rgba(124,58,237,0.18)"))
            fig_qr.add_trace(make_radar(CAREER_SKILLS[qtop], f"Ideal {qtop}", "#f59e0b", "rgba(245,158,11,0.08)"))
            ...
            for c, p in qr[:5]:
                ...
                <div style="margin-bottom:.65rem">
                  ...
                  <div style="background:var(--surface);border-radius:999px;height:7px">
                    <div style="width:{int(p)}%;height:100%;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--a2))"></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown('<div class="section-title"><span class="dot"></span> Career Explorer — click any card for a deep-dive & skill gap analysis</div>', unsafe_allow_html=True)

    careers_list = list(CAREER_ICONS.keys())
    rows = [careers_list[:5], careers_list[5:]]
    for ri, row in enumerate(rows):
        cols = st.columns(5)
        for ci, career in enumerate(row):
            with cols[ci]:
                is_sel = st.session_state.explorer_career == career
                label  = f"{CAREER_ICONS[career]}\n\n**{career}**\n\n{CAREER_DEMAND[career]}"
                if st.button(label, key=f"exp_{ri}_{ci}", use_container_width=True):
    ...
    if sel:
        ...
        cur   = {k: st.session_state.get(f"live_{k}", 5) for k in FEATURES}
        ...
        <div class="result-card" style="text-align:left;padding:1.5rem 2rem">
          <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;margin-bottom:.5rem">
            ...
            <div style="flex:1">
              <div style="font-size:1.6rem;font-weight:800;background:linear-gradient(90deg,#a78bfa,#06b6d4);
                -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">{sel}</div>
              <div style="color:#64748b;font-size:.85rem">{CAREER_DESC[sel]}</div>
            </div>
            <div style="text-align:right">
              <div style="font-family:Space Mono,monospace;color:var(--a3);font-size:.75rem">SALARY (INDIA)</div>
            ...
            for sk in FEATURES:
                ...
                <div style="margin-bottom:.6rem">
                  ...
                  <div style="background:var(--surface);border-radius:999px;height:7px;position:relative">
                    <div style="width:{cv*10}%;height:100%;border-radius:999px;background:linear-gradient(90deg,var(--accent),var(--a2))"></div>
                    <div style="position:absolute;top:-3px;left:{iv*10}%;width:3px;height:13px;background:var(--a3);border-radius:2px"></div>
                  </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with ec2:
            ...
            fig_e = go.Figure()
            fig_e.add_trace(make_radar(cur,   "Your Skills", "#7c3aed", "rgba(124,58,237,0.18)"))
            fig_e.add_trace(make_radar(ideal, f"Ideal {sel}", "#f59e0b", "rgba(245,158,11,0.1)"))
...
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="section-title"><span class="dot"></span> Side-by-Side Career Comparison</div>', unsafe_allow_html=True)
    ...
    if career_a == career_b:
        ...
        # ── Radar overlay ─────────────────────────────────────────────
        fig_cmp = go.Figure()
        fig_cmp.add_trace(make_radar(CAREER_SKILLS[career_a], career_a, "#7c3aed", "rgba(124,58,237,0.18)"))
        fig_cmp.add_trace(make_radar(CAREER_SKILLS[career_b], career_b, "#06b6d4", "rgba(6,182,212,0.12)"))
        ...
        sa = CAREER_SKILLS[career_a]
        sb = CAREER_SKILLS[career_b]
        h0, h1, h2, h3 = st.columns([2,1,.4,1])
        h0.markdown("**Skill**"); h1.markdown(f"**{CAREER_ICONS[career_a]} {career_a}**")
        h2.markdown("**vs**");   h3.markdown(f"**{CAREER_ICONS[career_b]} {career_b}**")
        st.markdown("---")
        ...
        msg = ("Very similar — skills transfer well! 🤝" if sim > 70
               else "Quite different — distinct strengths needed. 🔀" if sim < 50
               else "Moderate overlap — complementary paths. 🔄")
        st.markdown(f"""
        <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;
          ...
          <div style="color:var(--muted);font-size:.75rem;font-family:Space Mono,monospace;text-transform:uppercase;letter-spacing:.1em">Career Similarity Score</div>
...
with st.expander("ℹ️  About this model & dataset"):
    ...
    a1.markdown("**🤖 Algorithm**\n\nRandom Forest Classifier (200 trees) in a scikit-learn Pipeline with StandardScaler, serialised via **Joblib**.")
    a2.markdown("**📊 Dataset**\n\n1 000 synthetic profiles in **Excel (.xlsx)** covering 10 tech careers across 12 skill dimensions.")
    a3.markdown("**🎯 Accuracy**\n\n**88%** test accuracy · **85% ± 2.6%** 5-fold CV · Probabilities for all 10 career classes.")

# ── FOOTER ─────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="footer">
  Career Predictor AI · Streamlit + scikit-learn + Joblib · Excel Dataset · 88% Accuracy<br>
  4 Interactive Modes: Live Predictor · Skill Quiz · Career Explorer · Compare Careers
</div>
""", unsafe_allow_html=True)
