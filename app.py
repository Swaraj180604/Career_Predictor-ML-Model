...
# ── Auto-build model if .joblib files are missing (e.g. first deploy) ─────────
def build_and_save_model():
    """Train model from Excel dataset and persist with joblib."""
...
def load_models():
    if not os.path.exists(MODEL_PATH) or not os.path.exists(LE_PATH):
        st.info("⚙️ First run: training model from dataset… (takes ~5 s)")
        pipe, le = build_and_save_model()
    else:
        try:
...
# ── Page config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Career Predictor AI",
    page_icon="🚀",
    layout="wide",
...
/* ── SELECT ── */
.stSelectbox label{color:var(--muted)!important;font-size:.85rem!important;}
...
CAREER_SALARY = {
    ...
    "Cybersecurity Analyst":"₹8L – ₹30L","AI/ML Engineer":"₹12L – ₹50L",
    "Database Administrator":"₹6L – ₹25L","Network Engineer":"₹5L – ₹20L",
    "UI/UX Designer":"₹5L – ₹25L","Product Manager":"₹12L – ₹45L","DevOps Engineer":"₹8L – ₹32L",
}
CAREER_DEMAND = {
    "Software Engineer":"🔥 Very High","Data Scientist":"🔥 Very High","Web Developer":"✅ High",
    "Cybersecurity Analyst":"🔥 Very High","AI/ML Engineer":"🚀 Explosive",
    "Database Administrator":"✅ Moderate","Network Engineer":"✅ Moderate",
    "UI/UX Designer":"✅ High","Product Manager":"✅ High","DevOps Engineer":"🔥 Very High",
}
SKILL_LABELS = {
...
_defaults = {
    "quiz_step": 0,
    "quiz_scores": {k: 5 for k in FEATURES},
    "quiz_done": False,
...
def radar_trace(vals: dict, label: str, color: str, fill: str):
...
def radar_layout(h: int = 320) -> dict:
    return dict(
        polar=dict(
...
def prob_bars(results: list, height: int = 320, title: str = "All Career Match Probabilities"):
    careers = [c for c, _ in results]
    probs   = [p for _, p in results]
    colors  = ["#7c3aed" if i == 0 else "#252538" for i in range(len(results))]
    fig = go.Figure(go.Bar(
        x=probs,
        y=[f"{CAREER_ICONS[c]} {c}" for c in careers],
        orientation="h",
        ...
        for i, key in enumerate(FEATURES):
            with (rA if i % 2 == 0 else rB):
                values[key] = st.slider(
                    ...
                    int(st.session_state.get(f"live_{key}", 5)),
                    key=f"live_{key}",
                )
    ...
    with col_R:
        ...
        <div class="insight-grid">
          <div class="insight-card">
            ...
            <div class="insight-val">{SKILL_LABELS[top_sk].split(' ',1)[1]}</div>
          </div>
          <div class="insight-card">
            <div class="insight-icon">📈</div>
            <div class="insight-label">Avg Score</div>
            <div class="insight-val">{avg_s:.1f} / 10</div>
          </div>
          <div class="insight-card">
            <div class="insight-icon">🎯</div>
            <div class="insight-label">Grow This</div>
            <div class="insight-val">{SKILL_LABELS[weak_sk].split(' ',1)[1]}</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab2:
    ...
    if not st.session_state.quiz_done:
            ...
            with (oc1 if i % 2 == 0 else oc2):
                if st.button(opt, key=f"qopt_{step}_{i}", use_container_width=True):
                    for sk, boost in q["skill_map"][i].items():
                        st.session_state.quiz_scores[sk] = min(
                            10, st.session_state.quiz_scores[sk] + boost
                        )
                    if step + 1 >= total:
        ...
        with qc1:
            st.markdown(
                '<div class="section-title" style="margin-top:1.2rem"><span class="dot"></span>'
                " Your Quiz Profile vs Ideal</div>",
                unsafe_allow_html=True,
            )
            fig_qr = go.Figure()
        ...
        with b2:
            if st.button("📋  Copy Scores → Live Predictor", key="copy_scores"):
                for k, v in qs.items():
                    st.session_state[f"live_{k}"] = int(v)
...
with tab3:
    ...
    if sel:
        ...
        cur   = {k: int(st.session_state.get(f"live_{k}", 5)) for k in FEATURES}
        ...
        with ec1:
            st.markdown(
                '<div class="section-title" style="margin-top:1rem"><span class="dot"></span>'
                " Skill Gap: You vs Ideal</div>",
                unsafe_allow_html=True,
            )
            for sk in FEATURES:
                iv   = ideal[sk]
...
with tab4:
    ...
    with cc1:
        career_a = st.selectbox(
            "Career A", list(CAREER_ICONS.keys()), key="cmp_a",
            index=list(CAREER_ICONS.keys()).index(st.session_state.compare_a),
        )
    with cc2:
        st.markdown(
            '<div style="text-align:center;padding-top:2.3rem;font-size:1.5rem;color:var(--muted)">⚔️</div>',
            unsafe_allow_html=True,
        )
    with cc3:
        career_b = st.selectbox(
            "Career B", list(CAREER_ICONS.keys()), key="cmp_b",
    ...
    if career_a == career_b:
        ...
        st.markdown(
            '<div class="section-title"><span class="dot"></span> Skill-by-Skill Breakdown</div>',
            unsafe_allow_html=True,
        )
